...
\setmintedinline{escapeinside=||,mathescape=true}
...
\newcommandx{\BuiltInPattern}[3][1=$tag$, 2=$pv$, 3=true]{
	\ifthenelse{\boolean{#3}} {
		\mintinline{text}{
			BuiltInPattern(|#1|: BuiltInPatternKind, |#2|: str)
		}
	} {
		\mintinline{text}{
			BuiltInPattern(|#1|, |#2|)
...
\newcommandx{\PatternInHole}[4][1=$p_1$, 2=$p_2$, 3=$c_1$, 4=$c_n$]{
	\mintinline{text}{
		InHole(|#1|: Pattern, |#2|: Pattern, [|#3|,|$...$|,|#4|]: CheckConstraint?)
	}
}
...
\newcommandx{\CheckConstraintT}[3][1=$sym_1$, 2=$sym_2$, 3=true]{
	\ifthenelse{\boolean{#3}} {
		\mintinline{text}{
			CheckConstraint(|#1|: str, |#2|: str)
		}
	} {
		\mintinline{text}{
			CheckConstraint(|#1|, |#2|)
...
\newcommand{\TermInHole}{\lstinline[mathescape]!InHole($t_1$: TermTemplate, $t_2$: TermTemplate)! } 
...
\newcommand{\ApplyMetafunction}{\lstinline[mathescape]!ApplyMetafunction($mf$: string, $t_m$: TermTemplate)! } 
...
\newcommand{\RedexMatchAssertEqual}{\lstinline[mathescape]!RedexMatchAssertEqual($l$: string, $p$: Pattern, $t$: Term, $[m_1, ..., m_n]$: Match?)! } 

\newcommand{\TermLetAssertEqual}{\lstinline[mathescape]!TermLetAssertEqual([$(v_1, n_1, t_1), ..., (v_n, t_n, t_n)$]: Tuple(string, natural, Term)?, $t$: Term, $e$: Term)! }
...
\newcommand{\ReadFromStdinAndApplyReductionRelation}{\lstinline[mathescape]!ReadFromStdinAndApplyReductionRelation($red$: string, $mf$: string)! }
