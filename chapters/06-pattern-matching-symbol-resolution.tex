...
Given \LetDefineLanguage{$l$}, let $Nt_{l}$ be the set of all non-terminal symbols of the language. Given string $sym$, the prefix of $sym$ needs to be extracted. Let $prefix_{sym}$ be a string of characters up to the first occurrence of an underscore character in $sym$. There are several cases to consider.
...
\item $prefix_{sym} \in Nt_l$, return \NonTerminal[$prefix_{sym}$][$sym$][false].
...
Finally, if this transformation was performed on \LetDefineLanguage{$l$}, 
