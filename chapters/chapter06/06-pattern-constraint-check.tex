...
However, since current design of \texttt{Match} class explained in Section \ref{section:Match} does not allow assigning multiple terms to the same pattern-variable, and thus certain pattern variables have to be renamed. Given $n$ occurrences of a pattern-variable, $n-1$ occurrences are to be renamed. After completion of pattern matching these $n-1$ pattern-variables must be removed from all returned \texttt{Match} instances.
...
\item $p=$\LiteralPattern contains no assignable symbols and thus $p$ and $\emptyset$ are returned.
...
Finally, make annotation \MakeAnnotation{$p$}{"PatConstraintCheckRemoveVars"}{$R$}.

\subsection{\texttt{merge} Operation}
Before proceeding with pattern traversal, define \texttt{merge} operation. Given two sets $M_1$ and $M_2$ it produces two sets $M$ and $M_r$. Those sets are constructed in the following manner:
