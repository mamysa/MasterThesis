...
\item First, need to generate a list of expected terms \texttt{expected}, potentially empty.
	...
	\item Concatenate $f_{e_i}(Match())$ to \texttt{expected}.
	\end{enumerate}
\item Generate function $f_t$ for term-template $t$.
\item Let $f_{red}$ be a function generated after visiting \texttt{define-reduction-relation} with name $r$. Let \texttt{actual} =$f_{red}(f_t())$ be the list of terms after applying reduction relation.
